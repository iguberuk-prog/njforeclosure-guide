  ...
  for (const c of BLOG_COUNTIES) {
    ...
    posts.push(
      ...
      },
      {
        ...
        description: `Every stage of ${/^[AEIOU]/.test(c.name) ? "an" : "a"} ${c.name} County foreclosure in order — the Notice of Intention, the complaint, default, judgment, and the sale — with the deadline attached to each.`,
        tldr: `${/^[AEIOU]/.test(c.name) ? "An" : "A"} ${c.name} County foreclosure moves through fixed stages: 120+ days of delinquency before filing is generally permitted, a Notice of Intention at least 30 days before suit, a complaint with a 35-day answer window, then default or litigation, final judgment, and a sheriff sale that can generally be adjourned twice (30 days each). The full arc typically runs many months to more than a year — time that rewards homeowners who use each window deliberately.`,
        published,
        updated: published,
        minutes: 7,
        countyName: c.name,
